...
console.log('This script will test your Twilio configuration by sending a test SMS.');
console.log('');
...
async function runTest() {
  try {
    ...
    // Send test message
    console.log(`Sending test SMS from ${process.env.TWILIO_PHONE_NUMBER} to ${process.env.AMBULANCE_PHONE}...`);
    
    const message = await client.messages.create({
      body: 'This is a test message from your GodsEye emergency dispatch system.',
    ...
    if (error.message.includes('authenticate')) {
      ...
      console.log('- Make sure you\'re using the correct account SID (starts with "AC")');
    } else if (error.message.includes('not a valid phone') || error.code === 21211) {
      console.log('- The phone number format is invalid. Use E.164 format (e.g., +1XXXXXXXXXX)');
      console.log(`- Check the format of: ${process.env.AMBULANCE_PHONE}`);
